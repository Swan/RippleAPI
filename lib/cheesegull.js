...
module.exports.searchBeatmaps = async (searchQuery, gameMode, rankedStatuses, amount, offset) => {
    try {
        // In the event that no ammount or offset was given, have a default. 
        if (!amount) amount = 100;
        if (!offset) offset = 0;

        // In the event that rankedStatus & gameMode parameters but aren't of the correct type.
        if (rankedStatuses && !Array.isArray(rankedStatuses)) throw new Error('rankedStatuses must be an array if it is being used as a parameter.');

        let url = `http://storage.ripple.moe/api/search?query=${searchQuery}&amount=${amount}&offset=${offset}`;
        ...
        switch(gameMode) {
            ...
            case 'mania':
                ...
                break;
            case 'all':
                // Again, nothing really changes here, just an extra option to make code readable.
                break;
            default:
                throw new Error(`Invalid game mode: '${gameMode}' provided. You must provide either: 'osu', 'taiko', 'ctb', 'mania', or 'all'`);
        }        

        rankedStatuses.forEach((status) => {
            switch(status) {
                 case 'graveyard':
